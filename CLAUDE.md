...
- **`PMS Questions and Query - Sheet1.csv`** — the working copy actually being iterated on (question, 2 alt phrasings, required params, SQL, testing parameter, **and a `Status` column** added 2026-09-02: `PASS` / `FAIL - Fixed` / `Blocked - Data Not Available` / `Hold - Pending Clarification` / `N/A - SQL Not Written`). Edited directly as bugs are confirmed and fixed — see the "Confirmed correctness rules" and "Full validation pass" sections below before touching it. All 150 rows have been live-validated at least once; see the counts at the bottom of this file before re-running a full pass from scratch.
...
- `tbl_users.email` is the login identity; `user_id` is only known internally. **Every "my X" query in the workbook binds on `:current_user` (email), never `:current_user_id` (numeric)** — the caller (an end user or an LLM tool call) doesn't know their own numeric `user_id`. Resolve via `JOIN tbl_users u ON u.user_id = <fk>` and filter `WHERE u.email = :current_user`. As of 2026-09-02 the 8 rows that still used `:current_user_id` (rows 3, 4, 24, 25, 26, 46, 47, 58 — managed/owned projects, open/due/overdue tasks, hours logged today/in-range, allocations) have all been converted to this pattern and re-verified live; if a new "my X" row is added, follow the same join, not a raw `user_id` bind.
- `tbl_users` also holds `password`, `salt`, and `session_id` — never `SELECT *` from it in a query whose results get echoed back; select named non-credential columns instead (`user_id, email, username, first_name, last_name, enabled`). Note there is **no `status` column** on `tbl_users` — the active/enabled flag is `enabled`, not `status` (a query filtering `u.status = 1` will error).
- Confirmed test identities: `saurav.kaushik@intglobal.com` → `user_id = 3066` (individual contributor; username `Saurav2527`/`Saurav5846`, enabled=1). `tarak@intglobal.com` → `user_id = 43` (a project manager/owner on many projects — use for PM-facing "my managed projects" style tests). Canonical test project for project-scoped queries: **`WebRTC Solution Internal FC`** (`project_id = 14585`) — unlike most sampled projects it actually has non-zero `project_status_id`/`project_stage_id`/`rag`/`priority`, so status/stage joins resolve to real labels instead of null.
...
- Joining e.g. `tbl_project_tasks`, `tbl_risk`, `tbl_project_issues`, `tbl_timesheets` all directly off `tbl_projects` (or off `tbl_users`) in one query multiplies rows across every combination. This is not just a correctness bug — confirmed live twice more on 2026-09-02: a project-type delay/overrun query showed `overdue_projects` (3,543) exceeding `total_projects` (1,464) for the same group, and an allocation-vs-logged-hours query inflated a user's real 1,760 allocated hours to 107,360. **It is also a severe performance bug** — a "no recent activity" query joining 5 child tables (one of them entirely unused in the SELECT list) took **393 seconds** before being rewritten; the fan-out, not the data volume, was the cost. Pre-aggregate each dimension in its own subquery (or an independent correlated scalar subquery per row), then join the aggregates back to the parent — never join two or more "many" tables flat off the same parent and hope `DISTINCT`/`GROUP BY` will sort it out downstream.

**Two more unresolved items — don't silently resolve, ask first:**
- `tbl_projects.client_id` (`int`) joined to `tbl_clients.client_id` (`varchar(255)`) — type mismatch, not yet verified live whether it actually matches correctly. `client_name` came back NULL on every row tested through this join (2026-09-02) — consistent with the mismatch, but still not conclusively diagnosed.
- Risk-exposure "high" thresholds used ad hoc in some workbook queries (e.g. `rating >= 15`) don't match the app's own hardcoded banding (≥20 red / 12–19 amber / <12 green) — the correct banding model is still an open decision (C-12). As of 2026-09-02 these are bind params (`:exposure_threshold`, `:residual_threshold`, `:deal_value_threshold`) rather than hardcoded guesses, so the ambiguity is pushed to the caller instead of baked into the SQL — don't remove the bind param to "simplify" it back to a guess.

**Status-type label mismatch — a status master's "Closed"-sounding label is not necessarily the `status_type = 3` row:**
- Confirmed live 2026-09-02 on `tbl_project_task_status`: the row with `status_type = 3` (the documented "Closed" value above) is `task_status_id = 8`, labeled **"Live"** — not "Complete" (`id = 3`, which is `status_type = 0`). Six of the master's 13 statuses (`Complete`, `ReqComplete`, `ReTest`, `QAPass`, `Approve`, `OnHold`) share an undocumented `status_type = 0` catch-all.
- Same pattern on `tbl_project_issue_status` (checked on a 406-issue project): `status_type = 3` is the row labeled **"Complete"**; the literal label **"Closed"** is `status_type = 0`, along with `Acknowledged`/`Reopen`/`Dev Complete`/`QA Complete` — about 86% of that project's issues fell into the `status_type = 0` bucket.
- The developer-confirmed rules above (`status_type IN (1,2,4,6)` for open tasks, `status_type != 3` for open issues) are still correct **per value** and should not be changed — but be aware that in practice this means most custom mid-pipeline statuses count as "open" regardless of how final-sounding their label is. Never infer open/closed from a status's name; always check its `status_type` live before trusting it.

**`reference_no` on tasks, risks, and issues is a per-project sequence, not a globally unique ID:**
- Confirmed live 2026-09-02: 2,329 different projects each have their own task with `reference_no = '1'`; 9 projects share risk `reference_no = '1'`; 167 projects share issue `reference_no = '1'`. A query filtering only `WHERE t.reference_no = :task_reference` (or the risk/issue equivalent) without also scoping by `project_name`/`project_id` will silently match the wrong project's record whenever the reference number is a small, common one. Every task/risk/issue "detail lookup" query in the workbook now also requires `project_name` — keep that pairing on any new one.

**`tbl_projects.deal_value` and `rag` are effectively unpopulated across the whole database:**
- Confirmed live 2026-09-02: `deal_value` is `0` for all ~5,900 projects in the database, active or not. Any query weighting or filtering by deal value returns a functionally inert result — treat as `Blocked - Data Not Available` rather than debugging the SQL.
- Only 2 projects in the entire database have ever had `rag` set to `'Red'` or `'Amber'`, and neither is active — nearly the entire active portfolio's `rag` is blank. RAG-based active-portfolio breakdowns will be near-empty regardless of query correctness.

**MCP mysql server parser limitation — `AGG(DISTINCT ...)` is rejected even though it's valid MySQL:**
- Confirmed by bisection 2026-09-02: `SUM(DISTINCT pu.allocation_hrs)` fails with a generic `Parsing failed` error from the `mysql` MCP server's query validator, even though MySQL itself accepts it fine. This is a limitation of the MCP tool's own SQL parser, not a database error — don't spend time debugging the query's MySQL syntax when you see this. Workaround: restructure to avoid `DISTINCT` inside an aggregate, typically with an independent correlated scalar subquery per row instead of a join. This also tends to fix any join-fan-out the `DISTINCT` was papering over anyway.

## Full validation pass (2026-09-02)

All 150 rows in `PMS Questions and Query - Sheet1.csv` were run live against `engagedb` at least once and given a `Status`. Final counts: **73 PASS**, **43 FAIL - Fixed** (bug found and corrected in place), **21 N/A - SQL Not Written** (no SQL exists — mostly `NL2SQL Candidate` / `Needs SQL (Tool)` rows), **12 Blocked - Data Not Available**, **1 Hold - Pending Clarification**.

Of the 12 Blocked rows, only 5 have any SQL to run at all (rows 62, 63, 80, 110, 149 — all downstream of the unpopulated `allocation_from`/`allocation_to`/`deal_value`/`rag` columns above); they execute without error but return either zero rows or rows with meaningless zeroed/NULL computed columns. The other 7 (rows 21, 22, 23 — milestones; 56, 57 — timesheet approval state; 87 — project margin; 94 — risk actions due by date) have no SQL at all, because the underlying data structure doesn't exist in this schema: no milestone data, no timesheet-approval field, no cost/expense data to net against `deal_value` for a margin, and `tbl_risk.action` is free text with no due date of its own. None of these 12 are fixable by changing SQL — they need real data entry (allocation dates, deal value, rag) or new schema (milestones, timesheet approval, cost tracking, structured risk actions) before a query can be written or trusted.
